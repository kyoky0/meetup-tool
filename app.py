...
div[data-testid="stButton"] button {
    height: 72px; font-size: 20px; border-radius: 12px;
}
...
.timer-wrap {
    background: #f7f8fc; border: 2px solid #e2e8f0; border-radius: 14px;
    padding: 1rem; text-align: center; margin-bottom: 0.8rem;
}
.timer-label { font-size: 0.8rem; color: #718096; margin-bottom: 0.2rem; }
.timer-value { font-size: 52px; font-weight: bold; font-family: monospace; color: #1a202c; line-height: 1; }
.person-badge {
    background: #e8f4fd; border: 2px solid #4a90d9; border-radius: 50px;
    padding: 10px 20px; text-align: center; font-size: 18px;
    font-weight: bold; color: #2c5282; margin-bottom: 1rem;
}
.cut-log {
...
def transcribe_audio(audio_bytes: bytes, filename: str):
    """Groq Whisper で文字起こし → (segments, duration)"""
...
with tab2:
    ...
    # API キー（1つだけ）
    with st.expander("⚙️ API キー設定", expanded=not st.session_state.groq_key):
        groq_input = st.text_input(
            "Groq API Key",
            type="password",
            value=st.session_state.groq_key,
            placeholder="gsk_...",
            help="https://console.groq.com で無料取得できます",
        )
        if groq_input:
    ...
    can_process = audio_file is not None and bool(st.session_state.groq_key)
    ...
    if st.button("🚀 文字起こし・解析開始", type="primary", use_container_width=True, disabled=not can_process):
        ...
        try:
            ...
            # 3. バッチ解析（失敗時は個別にフォールバック）
            bar = st.progress(0.0, text="🤖 AI で解析中（一括処理）...")
            results = []
